...
// analyzer.
void TesseractImage(const char* input_file, IMAGE* image,
...
int main(int argc, char **argv) {
...
#ifndef _TIFFIO_
      "Warning - no libtiff - cannot read compressed tiff files.\n"
#endif
      , argv[0]);
  }
  ...
  api.Init(argv[0], lang);

  while (argc > arg) {
    api.ReadConfigFile(argv[arg++]);
  }
  ...
  if (len > 3 && strcmp("tif", argv[1] + len - 3) == 0) {
    ...
    do {
      ...
      TesseractImage(argv[1], &image, &api, &text_out);
